...
let has_duplicates (xs : string list) : bool =
  ...
  let rec check = function
    ...
    | _a :: _b :: _rest ->
      ...
      if _a = _b then true else check(_b::_rest)
  in
  check sorted
...
let format_assign (_a : assignment) : string =
  ...
  let name = _a.var_name in
  let value = string_of_int _a.value in
  let line = string_of_int _a.line in
...
let increment_value (_a : assignment) (_n : int) : assignment =
  ...
  {var_name = _a.var_name ; value = _a.value + _n ; line = _a.line}
...
let build_env (_pairs : (string * int) list) : int StringMap.t =
  ...
  StringMap.of_seq @@ List.to_seq _pairs
...
let get_key (_binding: (string * int)) : string = 
    let (s, _) = _binding in s

let all_vars (_env : int StringMap.t) : string list =
  (* EXERCISE: extract keys from StringMap.bindings *)
  let all_binds = StringMap.bindings _env in
   let keys : string list = List.map get_key all_binds in
   keys
...
let get_var_name(_a : assignment) : string = _a.var_name

let assigned_vars (_assignments : assignment list) : StringSet.t =
  (* EXERCISE: use List.fold_left and StringSet.add *)
  let list_of_var_names = List.map get_var_name _assignments in
  StringSet.of_list list_of_var_names
